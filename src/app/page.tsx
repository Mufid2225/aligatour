...
const trips = [
  {title:"Malang–Batu City Tour", place:"Malang & Kota Batu", price:"500.000", days:"Private · 4 kursi", rating:"4.9", image:"/malang-batu-city-tour.webp", tag:"Bebas pilih destinasi", href:"/trips/malang-batu-city-tour"},
  {title:"Ranu Kumbolo", place:"Lumajang, Jawa Timur", price:"749.000", days:"3H2M", rating:"4.9", image:"https://images.unsplash.com/photo-1602154663343-89fe0bf541ab?auto=format&fit=crop&w=1000&q=85", tag:"Paling diminati"},
  {title:"Tumpak Sewu", place:"Lumajang, Jawa Timur", price:"389.000", days:"2H1M", rating:"4.8", image:"https://images.unsplash.com/photo-1537996194471-e657df975ab4?auto=format&fit=crop&w=1000&q=85", tag:"Best seller"},
  {title:"Private Trip Bromo", place:"Meeting point Kota Malang", price:"1.800.000", days:"Private · Maks. 5 orang", rating:"4.9", image:"https://images.unsplash.com/photo-1588668214407-6ea9a6d8c272?auto=format&fit=crop&w=1000&q=85", tag:"3 pilihan paket", href:"/trips/private-trip-bromo"},
];
...
export default function Home() {
  return <main>
    <header className="absolute z-20 w-full py-5 text-white">
      <div className="container flex items-center justify-between">
        <a href="#" className="flex items-center gap-2.5" aria-label="Aligatour - Beranda">
          <Image src="/logo_aligatour.webp" alt="Logo Aligatour" width={58} height={58} priority className="h-[58px] w-[58px] object-contain" />
    ...
    <section id="trip" className="py-28"><div className="container">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">{trips.map((trip)=><article key={trip.title} className="trip-card overflow-hidden rounded-[24px] border border-[#dfe6df] bg-white">
        <div className="relative h-72"><Image src={trip.image} alt={trip.title} fill className="object-cover"/><span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1.5 text-xs font-extrabold text-[#176b5b]">{trip.tag}</span><span className="absolute bottom-4 right-4 flex items-center gap-1 rounded-full bg-[#153d37]/90 px-3 py-1.5 text-sm font-bold text-white"><Star weight="fill" className="text-[#d8f05a]"/> {trip.rating}</span></div>
        <div className="p-6"><div className="flex items-center gap-1.5 text-sm text-gray-500"><MapPin/>{trip.place}</div><h3 className="mt-2 text-2xl font-extrabold">{trip.title}</h3><div className="mt-5 flex items-end justify-between gap-3 border-t border-gray-100 pt-5"><div><span className="text-xs text-gray-500">Mulai dari</span><p><b className="text-xl text-[#176b5b]">Rp{trip.price}</b><span className="text-xs text-gray-400"> {trip.href ? "/grup" : "/orang"}</span></p></div><span className="rounded-full bg-[#f1f5e8] px-3 py-2 text-center text-xs font-bold">{trip.days}</span></div>{trip.href && <a href={trip.href} className="mt-5 flex items-center justify-center gap-2 rounded-full bg-[#153d37] px-5 py-3 text-sm font-bold text-white">Lihat detail paket <ArrowRight/></a>}</div>
      </article>)}</div>
    </div></section>
